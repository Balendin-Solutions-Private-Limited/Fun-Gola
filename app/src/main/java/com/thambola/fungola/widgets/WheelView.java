package com.thambola.fungola.widgets;
...
import com.thambola.fungola.SpinWinActivity;
...
public class WheelView extends View {
   ...
   // private MediaPlayer resourcePlayer;
    ...
    protected void initWheelView() {
        ...
        this.titlePaint.setColor(r.getColor(R.color.white));
        ...
        this.item8Paint.setColor(r.getColor(R.color.item10_color));
        this.item9Paint = new Paint(1);
        this.item9Paint.setColor(r.getColor(R.color.item9_color));
        this.item10Paint = new Paint(1);
        this.item10Paint.setColor(r.getColor(R.color.item8_color));
       ...
       // this.winner = "Click STOP to check your luck";
        this.winner = "";
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.itemNames != null) {
            ...
            float incr = 360.0f / ((float) this.itemNames.length);
            ...
            for (i = 0; i < this.itemNames.length; i++) {
                Paint itemPaint;
                switch (i) {
                    case Constants.KEY_ID_COLUMN /*0*/:
                        itemPaint = this.item1Paint;
                        break;
                    case Constants.NAME_COLUMN /*1*/:
                        itemPaint = this.item2Paint;
                        break;
                    case Constants.NAME_REF /*2*/:
                ...
                this.item = ((String) this.itemNames[i]).toString();
            ...
            if (this.lifted) {
                this.winner = ((String) this.itemNames[this.selected]).toString();
            }
          ...
          //  canvas.drawText(this.listName, (float) (this.px - (((int) this.titlePaint.measureText(this.listName)) / 2)), (((float) this.py) - this.radius) - ((float) (getMeasuredWidth() / 18)), this.titlePaint);
            ...
            if (this.lifted) {
                ...
                if (this.lastTheta > 0.0d) {
                    ...
                    if (this.lastTheta <= 0.0d) {
                        this.lifted = false;
                        SpinWinActivity.is_Spin=false;

                        ((SpinWinActivity)context).winnText(this.winner);
                    }
                } else {
                    currentTimeMillis = System.currentTimeMillis();
                    this.currTime = currentTimeMillis;
    ...
    public void autoSpin(float seed, boolean isspinstop) {

        //System.out.println("rrrrrrrrrrrrrrr seed "+seed);
      //  if (seed >= 20.0f) {
            seed = 15.0f;
      //  }
        this.isspinstop=isspinstop;
